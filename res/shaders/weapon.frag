...
void main(void)
{
	...
	vec4 finalColor = mix(weaponColor, reflectionColor, 0.02) * vec4(lightDot, lightDot, lightDot, 1.0);
	fragColor = mix(finalColor * 2.0, FOG_COLOR, dist) * vec4(lightDot, lightDot, lightDot, 1.0);
}
