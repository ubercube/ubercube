...
public class DeathPacket extends Packet
{
	...
	public void process(NetworkableClient client, InetAddress address, int port)
    {
        ...
        if (client.getCore().getGame().getEntityManager().getEntities().get(this.playerId) == null)
            return;

        client.getCore().getGame().spawn(new ParticleSystem(Indexer.getUniqueID(), "Death", ((Player) client.getCore().getGame().getEntityManager().getEntities().get(this.playerId)).getPosition())
