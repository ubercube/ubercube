...
public class WeaponPositionPacket extends Packet
{
	...
	{
		super(WEAPON_POS);
		this.data.put(this.clientID);
		this.data.put(this.weaponPosition);
		this.data.flip();
	}
	...
	public void read(DataBuffer data)
	{
		...
		this.weaponPosition = data.getInt();
	}

	@Override
	public void process(NetworkableServer server, InetAddress address, int port)
	{
		server.udpSendToAll(new WeaponPositionPacket(this.clientID, this.weaponPosition));
	}

	@Override
	public void process(NetworkableClient client, InetAddress address, int port)
	{
		Entity e = client.getCore().getGame().getEntityManager().get(this.clientID);
		if (e == null)
			return;
		...
		weapon.getWeapon().setPosition(this.weaponPosition);
	}
}