...
public class ECMouseLook extends EComponent
{
	...
	{
		Transform transform = ((ECRender) this.parent.get(EComponent.RENDER)).getTransform();

		float DY = this.dy * this.speed;

		if (this.rotAmnt + DY > 89)
		{
			DY = 89 - this.rotAmnt;
			this.rotAmnt = 89;

		}
		else if (this.rotAmnt + DY < -89)
		{
			DY = -89 - this.rotAmnt;
			this.rotAmnt = -89;
		}
		...
		transform.rotate(Vec3.UP, this.dx * this.speed);
