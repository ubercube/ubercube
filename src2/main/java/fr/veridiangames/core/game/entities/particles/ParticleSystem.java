...
public class ParticleSystem extends Entity
{
	...
	public void update(GameCore core)
	{
		...
		for(int i = 0; i < this.particles.size(); i++)
		{
			...
			if(this.collision && core.getGame().getWorld().getBlock((int) p.getTransform().getPosition().x, (int) p.getTransform().getPosition().y, (int) p.getTransform().getPosition().z) != 0)
				p.setCollision(true);
			else
				p.setCollision(false);

			if(!this.collision || !p.isCollision())
			{
	...
	public void destroy() {
		if(this.net != null)
        	this.net.send(new ParticlesRemovePacket(this), Protocol.TCP);
		else
			this.getCore().getGame().getEntityManager().remove(this.getID());
    }
	...
	{
		return this.particleColorRandomness;
	}
	...
	{
		return this.particleLifeTimeRandomness;
	}
	...
	{
		return this.particleVelocityRandomness;
	}
