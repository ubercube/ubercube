...
public abstract class Weapon
{
	...
	public void start()
	{
		...
		this.transform.getLocalPosition().add(this.hidePosition.getLocalPosition());
		this.transform.setLocalRotation(this.hidePosition.getLocalRotation());
	}
	...
	public void update(GameCore core)
	{
		if (this.currentPosition == 0)
			this.setTransformSmoothly(this.idlePosition, 0.4f);
		else if (this.currentPosition == 1)
			this.setTransformSmoothly(this.zoomPosition, 0.4f);
		this.rotationFactor.mul(0.7f);
		this.transform.setLocalRotation(Quat.euler(this.rotationFactor));
	}

	public void updateWeaponVelocity(Vec3 velocity, float dx, float dy, float factor)
	{
		this.rotationFactor.add(dy * factor, dx * factor, -dx * factor);
		this.rotationFactor.add(velocity.z * 0.2f, 0, velocity.x * 0.2f);
	}

	public void updateRunPosition()
	{
		this.rotationFactor.add(this.runRotation.copy().mul(0.01f));
	}
	...
	{
		this.bobbingTime++;
		float bobbing = Mathf.sin(this.bobbingTime * speed * 0.5f) * factor * velocity;
		float bobbingX = (Mathf.sin(this.bobbingTime * speed * 0.5f) * factor * velocity);
		if (bobbing * this.bobbingSide > 0)
			this.bobbingSide = -this.bobbingSide;
		this.rotationFactor.add(bobbing * this.bobbingSide, bobbingX, 0);
	}
	...
	public boolean isDrawed() { return this.drawed; }
}