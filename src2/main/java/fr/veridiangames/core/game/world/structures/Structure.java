...
{
	public static final Structure DESTROYED_BUILDING_0 = new Structure(ModelLoader.loadVox("src/main/res/structs/Bat1.vox"));
	...
	{
		int xSize = this.model.getxMax() - this.model.getxMin();
		int ySize = this.model.getyMax() - this.model.getyMin();
		int zSize = this.model.getzMax() - this.model.getzMin();
		...
		for (int x = 0; x < xSize; x++)
			for (int y = 0; y < ySize; y++)
				for (int z = 0; z < zSize; z++)
				{
					...
					world.addBlock(xx, yy, zz, this.model.getData()[x][y][z]);
				}
	}
}