...
public class WorldGen
{
	...
	public WorldGen(int height)
	{
		...
		this.heightData = new float[Chunk.SIZE][Chunk.SIZE];
	...
	public void addNoisePasses(WorldType type)
	{
		...
		{
			case SNOWY:
				this.add(new NoisePass(this.seed, 60, 5, 0, 1));
				this.add(new NoisePass(this.seed, 40, 10, 0.4f, -1));
				this.add(new NoisePass(this.seed, 40, 15, 0.45f, -1));
				break;

			case NORMAL:
				this.add(new NoisePass(this.seed, 60, 3, 0, 1));
				this.add(new NoisePass(this.seed, 20, 10, 0.4f, -1));
				this.add(new NoisePass(this.seed, 20, 20, 0, -1));
				break;
		}
	}
		...
		for (int xx = 0; xx < this.size; xx++)
			for (int yy = 0; yy < this.size; yy++)
			{
				...
				for (int x = 0; x < Chunk.SIZE; x++)
					...
					{
						float n = -10000;
						for (NoisePass p : this.noisePasses)
						{
							float nn = p.getNoisePass(x + xi, y + yi);
							if (nn > n) n = nn;
						}
						this.noise[x + xi][y + yi] = n;
					}
			}
	}
