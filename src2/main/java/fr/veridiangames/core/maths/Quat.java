...
//	}
	...
	public Quat sub(Quat value) {
		return new Quat(this.x - value.x, this.y - value.y, this.z - value.z, this.w - value.w);
	}
	...
	public Quat add(float value) {
		return new Quat(this.x + value, this.y + value, this.z + value, this.w + value);
	}
	...
	public Quat mul(Quat r) {
		float nw = this.w * r.w - this.x * r.x - this.y * r.y - this.z * r.z;
		float nx = this.x * r.w + this.w * r.x + this.y * r.z - this.z * r.y;
		float ny = this.y * r.w + this.w * r.y + this.z * r.x - this.x * r.z;
		float nz = this.z * r.w + this.w * r.z + this.x * r.y - this.y * r.x;

		return new Quat(nx, ny, nz, nw);
	}

	public Quat mul(Vec3 r) {
		float nw = -this.x * r.x - this.y * r.y - this.z * r.z;
		float nx = 	this.w * r.x + this.y * r.z - this.z * r.y;
		float ny = 	this.w * r.y + this.z * r.x - this.x * r.z;
		float nz = 	this.w * r.z + this.x * r.y - this.y * r.x;
	...
	public Mat4 toMatrix() {
		Vec3 forward = new Vec3(2.0f * (this.x * this.z - this.w * this.y), 2.0f * (this.y * this.z + this.w * this.x), 1.0f - 2.0f * (this.x * this.x + this.y * this.y));
		Vec3 up = new Vec3(2.0f * (this.x * this.y + this.w * this.z), 1.0f - 2.0f * (this.x * this.x + this.z * this.z), 2.0f * (this.y * this.z - this.w * this.x));
		Vec3 right = new Vec3(1.0f - 2.0f * (this.y * this.y + this.z * this.z), 2.0f * (this.x * this.y - this.w * this.z), 2.0f * (this.x * this.z + this.w * this.y));
...
//	}
	...
	public Quat(Mat4 rot) {
		...
		if (trace > 0) {
			...
			this.w = 0.25f / s;
			this.x = (rot.matrix[1][2] - rot.matrix[2][1]) * s;
			this.y = (rot.matrix[2][0] - rot.matrix[0][2]) * s;
			this.z = (rot.matrix[0][1] - rot.matrix[1][0]) * s;
		} else if (rot.matrix[0][0] > rot.matrix[1][1] && rot.matrix[0][0] > rot.matrix[2][2]) {
			float s = 2.0f * (float) Math.sqrt(1.0f + rot.matrix[0][0] - rot.matrix[1][1] - rot.matrix[2][2]);
			this.w = (rot.matrix[1][2] - rot.matrix[2][1]) / s;
			this.x = 0.25f * s;
			this.y = (rot.matrix[1][0] + rot.matrix[0][1]) / s;
			this.z = (rot.matrix[2][0] + rot.matrix[0][2]) / s;
		} else if (rot.matrix[1][1] > rot.matrix[2][2]) {
			float s = 2.0f * (float) Math.sqrt(1.0f + rot.matrix[1][1] - rot.matrix[0][0] - rot.matrix[2][2]);
			this.w = (rot.matrix[2][0] - rot.matrix[0][2]) / s;
			this.x = (rot.matrix[1][0] + rot.matrix[0][1]) / s;
			this.y = 0.25f * s;
			this.z = (rot.matrix[2][1] + rot.matrix[1][2]) / s;
		} else {
			float s = 2.0f * (float) Math.sqrt(1.0f + rot.matrix[2][2] - rot.matrix[0][0] - rot.matrix[1][1]);
			this.w = (rot.matrix[0][1] - rot.matrix[1][0]) / s;
			this.x = (rot.matrix[2][0] + rot.matrix[0][2]) / s;
			this.y = (rot.matrix[1][2] + rot.matrix[2][1]) / s;
			this.z = 0.25f * s;
		}
