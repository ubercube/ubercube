...
public class LoadingScreen extends GuiCanvas
{
    ...
    public LoadingScreen(GuiCanvas parent, Display display)
    {
        ...
        this.loadingInfo = new GuiLabel("Connecting...", display.getWidth() / 2, display.getHeight() - 80, 20f);
        this.loadingInfo.setOrigin(GuiComponent.GuiOrigin.CENTER);
        this.loadingInfo.setScreenParent(GuiComponent.GuiCorner.BC);
        super.add(this.loadingInfo);

        this.joinGameButton = new GuiButton("Join game", display.getWidth() / 2, display.getHeight() - 50, 200, new GuiActionListener()
        {
            @Override
			public void onAction()
            {
                LoadingScreen.this.loadingInfo.setText("Joining game...");
                LoadingScreen.this.joinGame = true;
            }
        });
        this.joinGameButton.centerText();
        this.joinGameButton.setClickable(false);
        this.joinGameButton.setOrigin(GuiComponent.GuiOrigin.CENTER);
        this.joinGameButton.setScreenParent(GuiComponent.GuiCorner.BC);
        super.add(this.joinGameButton);
    ...
    public void update(Ubercube main)
    {
        ...
        {
            this.loadingInfo.setText("Connected !");
            this.joinGameButton.setClickable(true);
        }
    }
