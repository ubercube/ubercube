...
public class Camera
{
	...
	{
		Mat4 translationMatrix = Mat4.translate(-this.transform.getPosition().x, -this.transform.getPosition().y, -this.transform.getPosition().z);
		Mat4 rotationMatrix = Mat4.rotate(this.transform.getRotation().getForward(), this.transform.getRotation().getUp());
		Mat4 projectionMatrix = Mat4.perspective(this.fov, this.aspect, this.near, this.far);
	...
	{
		this.aspect = this.display.getAspect();
		this.distanceCulling.update(this.transform.getPosition(), distance);
		this.frustumCulling.update(this);
	}

	public boolean isInViewDistance(Vec3 pos, float radius)
	{
		return this.distanceCulling.isInViewDistance(pos, radius);
	}

	public boolean isInViewFrustum(Vec3 pos, float radius)
	{
		return this.frustumCulling.isInViewFrustum(pos, radius);
	}
