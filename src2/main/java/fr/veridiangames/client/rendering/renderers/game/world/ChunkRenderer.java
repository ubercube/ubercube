...
public class ChunkRenderer
{
	...
	public void createBuffer()
	{
		...
		this.cbo = Buffers.createVertexBuffer();
	...
	public void updateChunk()
	{
		...
		for (int x = 0; x < Chunk.SIZE; x++)
			for (int y = 0; y < Chunk.SIZE; y++)
				...
				{
					int block = this.chunk.blocks[x][y][z];
					if (block == 0)
						continue;

					int xx = this.chunk.position.x * Chunk.SIZE + x;
					int yy = this.chunk.position.y * Chunk.SIZE + y;
					int zz = this.chunk.position.z * Chunk.SIZE + z;

					int upBlock = this.world.getBlock(xx, yy + 1, zz);
					int downBlock = this.world.getBlock(xx, yy - 1, zz);
					int leftBlock = this.world.getBlock(xx - 1, yy, zz);
					int rightBlock = this.world.getBlock(xx + 1, yy, zz);
					int frontBlock = this.world.getBlock(xx, yy, zz - 1);
					int backBlock = this.world.getBlock(xx, yy, zz + 1);
					...
					float brightness = 1;
					int size = this.calcBlock(vertexBuffer, xx, yy, zz, block, brightness, up, down, left, right, front, back);
	...
	{
		this.isInViewDistance = camera.isInViewDistance(this.chunk.centerPosition, 48);
		this.isInViewFrustum = camera.isInViewFrustum(this.chunk.centerPosition, 16);
	}
	...
	public void render()
	{
		...
		if (!this.isInViewDistance)
			return;
		if (!this.isInViewFrustum)
			return;

		glBindVertexArray(this.vao);
		glDrawArrays(GL_QUADS, 0, this.bufferSize);
		glBindVertexArray(0);
	}

	public void dispose()
	{
		Buffers.deleteVertexArray(this.vao);
		Buffers.deleteVertexBuffer(this.vbo);
		Buffers.deleteVertexBuffer(this.cbo);
	}
	...
	private int calcBlock(FloatBuffer buffer, int xx, int yy, int zz, int block, float brightness, boolean up, boolean down, boolean left, boolean right, boolean front, boolean back)
	{
		...
		if (up)
		{
			...
			if (this.enableAO)
			{
				if (this.world.getBlock(xx - 1, yy + 1, zz) != 0)
				{
					shading[0] *= ao;
					shading[3] *= ao;
				}
				if (this.world.getBlock(xx - 1, yy + 1, zz - 1) != 0)
					shading[0] *= ao;
				if (this.world.getBlock(xx, yy + 1, zz - 1) != 0)
				{
					shading[0] *= ao;
					shading[1] *= ao;
				}
		...
		if (left)
		{
			...
			if (this.enableAO)
			{
				...
				if (this.world.getBlock(xx - 1, yy - 1, zz - 1) != 0)
					shading[0] *= ao;
				if (this.world.getBlock(xx - 1, yy, zz - 1) != 0)
				{
					shading[0] *= ao;
					shading[1] *= ao;
				}
