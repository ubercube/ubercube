...
public class PlayerRenderer
{
	...
	public PlayerRenderer()
	{
		...
		for (int i = 0; i < MAX_ENTITIES; i++)
		{
			...
			this.instanceBuffer.put(Color4f.BLACK.toArray());
			this.instanceBuffer.put(Mat4.identity().getComponents());
			this.instanceBuffer.put(Color4f.BLACK.toArray());
		}
	...
	public void updateInstances(Map<Integer, Entity> entities, List<Integer> indices)
	{
		...
		for (int i = 0; i < indices.size(); i++)
		{
			...
			this.instanceBuffer.put(Mat4.translate(transform.getPosition()).mul(Mat4.scale(0.35f, 1.0f, 0.35f)).getComponents());
			this.instanceBuffer.put(Color4f.GRAY.toArray());
			this.instanceBuffer.put(Mat4.translate(transform.getPosition().copy().add(0, 2.5f / 2.0f, 0)).mul(transform.getRotation().toMatrix()).mul(Mat4.scale(0.4f, 0.4f, 0.4f)).getComponents());
			this.instanceBuffer.put(Color4f.DARK_GRAY.toArray());
		}
		this.instanceBuffer.flip();
		glBindBuffer(GL_ARRAY_BUFFER, this.vio);
		glBufferData(GL_ARRAY_BUFFER, this.instanceBuffer, GL_DYNAMIC_DRAW);
	...
	{
		glBindVertexArray(this.vao);
		glDrawElementsInstanced(GL_TRIANGLES, this.cubeIndices().length, GL_UNSIGNED_INT, 0L, this.renderCount * ENTITY_COMPLEXITY);
		glBindVertexArray(0);
	}
