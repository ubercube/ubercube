...
public class PlayerNameRenderer
{
    ...
    {
        this.font = new TrueTypeFont(StaticFont.Kroftsmann(0, 30), true);
        this.playerFontRenderers = new ArrayList<>();
    }

    public void update(Map<Integer, Entity> entities, java.util.List<Integer> indices)
    {
        this.playerFontRenderers.clear();
        for (int i = 0; i < indices.size(); i++)
        {
            ...
            Font3DRenderer renderer = new Font3DRenderer(this.font, name, position.copy().add(0, 2.3f, 0));
            if (team != null)
                renderer.setColor(team.getColor());
            this.playerFontRenderers.add(renderer);
        }
    }

    public void render(Gui3DShader shader, Camera camera)
    {
        for (int i = 0; i < this.playerFontRenderers.size(); i++)
        {
            Color4f color = this.playerFontRenderers.get(i).getColor();
            this.playerFontRenderers.get(i).render(shader, camera, color, 4);
        }
    }
}
